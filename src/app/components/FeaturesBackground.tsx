...
export function FeaturesBackground() {
  ...
  useEffect(() => {
    setIsMounted(true);
    const checkMobile = () => {
      setIsMobile(window.innerWidth < 1024 || /Android/i.test(navigator.userAgent));
    };
    checkMobile();
  ...
  const largeParticleCount = isMobile ? 8 : 35;
  const horizontalParticleCount = isMobile ? 5 : 25;
  ...
  const largeParticles = useMemo(() => {
    if (!isMounted) return [];
    return Array.from({ length: largeParticleCount }, () => ({
  ...
  }, [isMounted, largeParticleCount]);

  const horizontalParticles = useMemo(() => {
    if (!isMounted) return [];
    return Array.from({ length: horizontalParticleCount }, () => ({
      left: Math.random() * 100,
      top: Math.random() * 100,
      duration: 4 + Math.random() * 3,
      delay: Math.random() * 6,
    }));
  }, [isMounted, horizontalParticleCount]);
  ...
  if (!isMounted) {
    return <div className="absolute inset-0 overflow-hidden bg-white" />;
  }

  return (
    <div className="absolute inset-0 overflow-hidden bg-white">
      {/* SVG Grid Layer 1 - 80px spacing - Disabled on mobile for performance */}
      {!isMobile && (
      ...
      {/* SVG Grid Layer 2 - 60px spacing with offset timing - Disabled on mobile */}
      {!isMobile && (
      ...
      {/* Animated black gradient orbs - Static on mobile */}
      {!isMobile && (
        <>
          <motion.div
      ...
      {/* Static orbs on mobile for visual consistency without animation overhead */}
      {isMobile && (
        <>
          <div className="absolute top-1/4 left-1/4 w-96 h-96 bg-black/8 rounded-full blur-3xl opacity-40" />
          <div className="absolute bottom-1/4 right-1/4 w-80 h-80 bg-black/6 rounded-full blur-3xl opacity-40" />
        </>
      )}

      {/* Sparkles - floating particles - Reduced count on mobile */}
      {particles.map((particle, i) => (
        <motion.div
          ...
          animate={{
            y: [0, -80 - (particle.left % 40), 0],
            opacity: [0, 0.6, 0],
            scale: [0, 1 + (particle.top % 50) / 100, 0],
          }}
          transition={{
            duration: isMobile ? particle.duration * 2 + 2 : particle.duration,
            repeat: Infinity,
      ...
      {/* Larger sparkles - Reduced count on mobile */}
      ...
      {/* Horizontal floating particles - Reduced count on mobile */}
      {horizontalParticles.map((particle, i) => (
        <motion.div
          key={`horizontal-${i}`}
          className="absolute w-1 h-1 bg-black rounded-full"
          style={{
      ...
      {/* Scanning lines - Disabled on mobile for performance */}
      {!isMobile && (
        <>
      ...
      {/* Animated rings - Reduced count on mobile */}
      {rings.map((ring, i) => (
        <motion.div
          key={`ring-${i}`}
      ...
      {/* Pulsing dots - Reduced count on mobile */}
      {dots.map((dot, i) => (
        <motion.div
          key={`dot-${i}`}
